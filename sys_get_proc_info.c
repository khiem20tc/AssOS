...
{
	printk("Student ID: 1810998");
	info->studentID = 1810998;
	if (pid == -1) {
		pid = current->pid;
	}
	...
	for_each_process (proces) {
		if (proces->pid == pid) {
			...
			if (proces->real_parent == NULL) {
				...
				//info->patent_proc.name = proces->real_parent->comm;
				strcpy(info->parent_proc.name,
			...
			if (cProces == NULL) {
				...
				strcpy(info->oldest_child_proc.name,
				       cProces->comm);
			}
		}
	}
